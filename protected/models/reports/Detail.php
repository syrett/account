...
class Detail extends CModel
{
  ...
  private function getTransition($sbj_id, $year, $fm, $tm){
    $sql = "SELECT entry_num_prefix, entry_num, entry_transaction, entry_amount,entry_memo, date(entry_date) FROM transition WHERE entry_subject=:subject_id AND year(entry_date) =:year AND month(entry_date) >= :fm AND Month(entry_date)<=:tm ORDER BY entry_num_prefix";
    ...
    foreach($transition as $row) {
      ...
      case 1:
        ...
        $arr["balance"] = balance($this->balance, $row["entry_amount"], 0, $this->sbj_cat);
        $this->sum_debit += $row["entry_amount"];
        break;
      case 2:
        $arr["credit"] = $row["entry_amount"];
        $arr["balance"] = balance($this->balance, 0, $row["entry_amount"], $this->sbj_cat);
