...
namespace vova07\bank\controllers\backend;
...
use vova07\bank\models\backend\Bank;
use vova07\bank\models\backend\BankSearch;
...
//        }
    ...
    public function actionAdmin(){
        ...
        $sheetData = $searchModel->search();
        return $this->render('admin', ['sheetData' => $sheetData]);
    }
    ...
    public function actionUpdate($id){
        ...
        Yii::$app->response->redirect('/index.php?r=bank/update&id='. $id);
    }
    ...
    {
        if (is_array($id)) {
            /** @var \vova07\bank\models\backend\Bank $model */
            $model = Bank::findAll($id);
        } else {
            /** @var \vova07\bank\models\backend\Bank $model */
    ...
    protected function findModels($id)
    {
        ...
        if ($model !== null && $model->parent!==null) {
            return $search->findCorrelation($model->parent, $id);
        }
    }
    ...
    {
        $this->saveAll();
        Yii::$app->response->redirect('/index.php?r=Transition/bank&action=save');
    }
    ...
    protected function saveAll($id=''){
        ...
        foreach ($trans as $key => $item) {
            ...
            elseif(isset($item['Transition']['id']))
                $bank = Bank::findOne($item['Transition']['id']);
            else
                $bank = new Bank();
            $bank->load($_POST['lists'][$key]);
