...
function itemSet() {
    ...
    $("#tax_" + item_id).val($("#new-category-3").val());
    setTax(item_id);    //设置税
    setTarget(item_id);
...
//设置按钮宽度
function setWidth(e){
    var width = Math.max.apply(Math, $(e.parentNode.nextSibling.nextSibling).find('button[class*=btn-primary]').map(function(){ return $(this).width(); }).get());
