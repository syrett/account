...
员工报销导入时，如果员工不在员工列表里，要提示；
导入生成的凭证在查看时，由于凭证不能选择银行现金科目，导致凭证查看时科目不准确；
...
/*
员工报销时，分批次报销金额等于剩余金额时，科目应该计入其他应付
结转时，如果是当年12月，多生成一条凭证，借本年利润，贷利润分配/未分配利润；
坏账处理；
账号申请加入同一个组；
固定资产无形资产 总账期初余额和明细的计算，把已经折旧的金额也计算在内；
员工报销导入时，如果员工不在员工列表里，要提示；        todo::林强
导入生成的凭证在查看时，由于凭证不能选择银行现金科目，导致凭证查看时科目不准确；    todo::林强
