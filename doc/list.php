...
/*
 * 银行现金记账时，没有找到匹配时，仍然没有显示所有选项；           todo:: 林强
 * 现金流量表，利息收入单独放在其他经营性收出；
 * 现金流量表，主营业务成本从其他经营活动转入劳务支付的现金；
 * 固定资产无形资产 总账期初余额和明细的计算，把已经折旧的金额也计算在内；
 * 标准版，年终奖的设置，每月单独计算税；
 * 财务类报表，增值税纳税申报表、营业税、个人所得税、企业所得税；
 * 审核凭证没有权限时，没有提示；                          todo:: 林强
 * 删除银行现金时，如果没有权限，不显示403数字，只显示中文；           todo:: 林强
 * 客户，供应商 账龄的权限控制，大众版和标准版权限内容不相同，销售的坏账放在销售模块；
 * 权限控制时，检测被编辑用户是否和当前登录用户在同一个组；           todo:: 林强
 * 账号权限，账套所有者的权限不能修改；           todo:: 林强
 * 权限，总账期初余额，可以查看，客户和供应商类似；
 * 员工报销时，分批次报销金额等于剩余金额时，科目应该计入其他应付？？？未找到bug重现情况
 * 账套没有权限时，账套首页也不能查看；           todo:: 林强
 *
 * 注册时改善确认注册邮件才可用，QQ登录确认常用邮箱；
 * 账号申请加入同一个组；
 *
 * 账套首页添加，信息中心、财务汇总（三个饼图，运营收入、运营支出、管理费用（二级科目））、税务中心
